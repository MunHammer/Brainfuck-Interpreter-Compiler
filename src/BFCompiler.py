...
class Compiler:
    def __init__(
        ...
        *,
        ...
        indent: int = 0,
        autosemicolon: bool = False,
        autonewline: bool = True,
    ):
    ...
    def add_command(
        ...
        *,
        ...
        match command:
            ...
            case "," if self.__commands[","] == "" or overwrite:
                self.__commands[","] = compiled
            case "increment" if self.__commands["increment"] == "" or overwrite:
                self.__commands["increment"] = compiled
            case "move" if self.__commands["move"] == "" or overwrite:
                self.__commands["move"] = compiled
            case "#" if self.__commands["#"] == "" or overwrite:
                self.__commands["#"] = compiled
            case "addcheck" if self.__commands["addcheck"] == "" or overwrite:
                self.__commands["addcheck"] = compiled
            case "movecheck" if self.__commands["movecheck"] == "" or overwrite:
                self.__commands["movecheck"] = compiled
            case _:
                raise ValueError(
                    "Class method: add_command() requires "
            ...
            match command.type:
                ...
                case "o" if command.num < 0:
                    try:
                        ...
                        self.result += self.__commands[","]
                case "i":
                    self.result += self.__commands["increment"].format(Num=command.num)
                    self.__check_add("add")
                case "m":
                    self.result += self.__commands["move"].format(Num=command.num)
...
def makeCPP():
    ...
    CPPcompiler.add_opening("#include <iostream>", semicolon=False)
    CPPcompiler.add_opening("#include <stdexcept>", semicolon=False)
    CPPcompiler.add_opening("using namespace std")
    CPPcompiler.add_opening("void movecheck(int ptr) {", semicolon=False)
    CPPcompiler.add_opening("if (ptr < 0 or ptr > 30000) {", indent=1, semicolon=False)
    CPPcompiler.add_opening(
        'throw out_of_range("No negative cells or cells > 30,000 exist")', indent=2
    )
    CPPcompiler.add_opening("}", semicolon=False, indent=1)
    CPPcompiler.add_opening("}", semicolon=False)
    CPPcompiler.add_opening("int addcheck(int cell) {", semicolon=False)
    CPPcompiler.add_opening("while (cell > 255) {", indent=1, semicolon=False)
    CPPcompiler.add_opening("cell -= 256", indent=2)
    CPPcompiler.add_opening("}", semicolon=False, indent=1)
    CPPcompiler.add_opening("while (cell < 0) {", indent=1, semicolon=False)
    CPPcompiler.add_opening("cell += 256", indent=2)
    CPPcompiler.add_opening("}", semicolon=False, indent=1)
    CPPcompiler.add_opening("return cell", indent=1)
    CPPcompiler.add_opening("}", semicolon=False)
    CPPcompiler.add_opening("int main() {", semicolon=False)
    CPPcompiler.add_opening("int tape[30000]={0}", indent=1)
    CPPcompiler.add_opening("int ptr = 0", indent=1)
    ...
    CPPcompiler.add_command("addcheck", compiled="tape[ptr] = addcheck(tape[ptr])")
...
def makePY():
    ...
    PYcompiler.add_opening(
        'raise Exception("No memory adresses exist beyond 0 & 30,000")', indent=2
    )
...
def makeRust():
    ...
    Rustcompiler.add_opening(
        "fn inc(cell: &mut u8, incrementation: i16) {", semicolon=False
    )
    Rustcompiler.add_opening("if incrementation < 0 {", indent=1, semicolon=False)
    Rustcompiler.add_opening(
        "*cell = cell.wrapping_sub(incrementation as u8)", indent=2
    )
    ...
    Rustcompiler.add_command(
        ".", 'print!("{}", tape[ptr] as char); std::io::stdout().flush().unwrap()'
    )
